...
export default class MakeMigrationAlterTable extends BaseMakeFile {
  constructor () {
    super('migrations')
  }
  ...
  public async run (dir: string, options: FileOptions): Promise<void> {
    ...
    const { templateFile, targetFile, filenameUpper, location } = await prepareFile(`${this.type}/UpdateTable`, dir)
